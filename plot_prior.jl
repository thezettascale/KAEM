...
fcn_type = "RBF"
datasets = ["DARCY_FLOW", "MNIST", "FMNIST"]
...
for dataset_name in datasets
    ...
    sorted_indices = sortperm(vec(alpha), rev=true)[1:3]
    plot_components = [(div(idx-1, size(alpha,2)) + 1, mod(idx-1, size(alpha,2)) + 1) for idx in sorted_indices]
