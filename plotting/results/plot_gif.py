...
GIF_CONFIGS = {
    ...
    "DARCY_FLOW_gaussian_fft": {
        ...
        "grid_size": 6,
        "cmap": "viridis",
        "epochs": [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000],  # Darcy uses 100-step increments
        "samples_per_frame": 6,
...
def create_sample_subset_frame(config, epoch, sample_subset_idx, generated_images, total_samples):
...
def create_evolution_gif(config):
    print(f"Creating GIF for {config['dataset']} - {config['prior']} - {config['function']}")
    ...
    if not frames:
        print(f"  No valid frames found for {config['dataset']} - {config['prior']} - {config['function']}")
        return
