...
function discriminator_loss(ps_disc, x_real, x_fake, disc, st_disc)
    ...
    loss_real = Flux.logitbinarycrossentropy(logits_real, one(eltype(logits_real)))
    loss_fake = Flux.logitbinarycrossentropy(logits_fake, zero(eltype(logits_fake)))
...
function generator_loss(ps_gen, z, gen, disc, ps_disc, st_gen, st_disc)
    ...
    loss = Flux.logitbinarycrossentropy(logits_fake, one(eltype(logits_fake)))
...
function (l::GANTrainStep)(opt_state_gen, opt_state_disc, ps, st, x_real, z, train_idx)
    ...
    d_loss, st_disc = discriminator_loss(
    ...
    if train_idx % l.n_critic == 0
        ∇_gen = grad_gen(ps.gen, z, gen, disc, ps_disc_new, st.gen, st_disc)
        g_loss, st_gen, _ = generator_loss(
            ps.gen, z, gen, disc, ps_disc_new, Lux.trainmode(st.gen), st_disc
        )
        opt_state_gen, ps_gen_new = Optimisers.update(opt_state_gen, ps.gen, ∇_gen)
    end

    ps_new = (gen = ps_gen_new, disc = ps_disc_new)
    st_new = (gen = st_gen, disc = st_disc)

    return d_loss + g_loss, ps_new, opt_state_gen, opt_state_disc, st_new
end
