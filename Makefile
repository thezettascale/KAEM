...
ifeq ($(MODE),thermo)
	@tmux send-keys -t tkam_train:training "export LD_LIBRARY_PATH=\$$HOME/.julia/artifacts/2eb570b35b597d106228383c5cfa490f4bf538ee/lib:\$$LD_LIBRARY_PATH && if [ -f '$(CONDA_ACTIVATE)' ]; then . '$(CONDA_ACTIVATE)' && conda activate $(ENV_NAME) && DATASET=$(DATASET) julia --project=. --threads=auto --gcthreads=1 main_thermodynamic.jl 2>&1 | tee logs/train_thermo_$(DATASET)_$(shell date +%Y%m%d_%H%M%S).log; else conda activate $(ENV_NAME) && DATASET=$(DATASET) julia --project=. --threads=auto --gcthreads=1 main_thermodynamic.jl 2>&1 | tee logs/train_thermo_$(DATASET)_$(shell date +%Y%m%d_%H%M%S).log; fi" Enter
else
	@tmux send-keys -t tkam_train:training "export LD_LIBRARY_PATH=\$$HOME/.julia/artifacts/2eb570b35b597d106228383c5cfa490f4bf538ee/lib:\$$LD_LIBRARY_PATH && if [ -f '$(CONDA_ACTIVATE)' ]; then . '$(CONDA_ACTIVATE)' && conda activate $(ENV_NAME) && DATASET=$(DATASET) julia --project=. --threads=auto --gcthreads=1 main.jl 2>&1 | tee logs/train_vanilla_$(DATASET)_$(shell date +%Y%m%d_%H%M%S).log; else conda activate $(ENV_NAME) && DATASET=$(DATASET) julia --project=. --threads=auto --gcthreads=1 main.jl 2>&1 | tee logs/train_vanilla_$(DATASET)_$(shell date +%Y%m%d_%H%M%S).log; fi" Enter
endif
...
plot:
	...
	@export LD_LIBRARY_PATH=$$HOME/.julia/artifacts/2eb570b35b597d106228383c5cfa490f4bf538ee/lib:$$LD_LIBRARY_PATH && $(call conda_run,./scripts/run_plots.sh 2>&1 | tee logs/plotting_$(shell date +%Y%m%d_%H%M%S).log)
	@echo "Plotting completed! Log file: logs/plotting_$(shell date +%Y%m%d_%H%M%S).log"

plot-results:
	@mkdir -p logs
	@echo "Running results plotting scripts..."
	@export LD_LIBRARY_PATH=$$HOME/.julia/artifacts/2eb570b35b597d106228383c5cfa490f4bf538ee/lib:$$LD_LIBRARY_PATH && $(call conda_run,find plotting/results/ -name "*.py" -exec python {} \; 2>&1 | tee logs/plotting_results_$(shell date +%Y%m%d_%H%M%S).log)
