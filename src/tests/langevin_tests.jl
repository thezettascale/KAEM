...
function plot_final_distribution()
    ...
    final_samples = vec(output[2, :, 1])
    histogram(final_samples, bins=50, normalize=true, alpha=0.6, label="Samples")
    
    μ, σ = 0.0, 1.0  # Expected Gaussian
    x = range(minimum(final_samples), maximum(final_samples), length=100)
    plot!(x, pdf.(Normal(μ, σ), x), lw=2, label="Expected Posterior")
    plot!(legend=:topleft, xlabel="z", ylabel="Density")
    savefig("figures/test/langevin_convergence.png")
