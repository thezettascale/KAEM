...
for (idx, (df, ref, title)) in enumerate(zip(datasets, references, titles)):
    ...
    if ref is not None:  
        if idx == 1:  # MALA steps plot
            axs[0, 0].axhline(y=ref["Time (s)"], color='red', linestyle='--', 
    ...
    if ref is not None:  
        if idx == 1:  # MALA steps plot
            axs[0, 1].axhline(y=ref["Memory Estimate (GiB)"], color='red', linestyle='--',
                              label='Reference MLE criterion\nwith single posterior\nULA sampling')
        else:
            axs[0, 1].axhline(y=ref["Memory Estimate (GiB)"], color='red', linestyle='--',
                              label='Reference ITS\nwith Mixture Prior')
    ...
    if ref is not None:
        if idx == 1:  # MALA steps plot
            axs[1, 0].axhline(y=ref["Garbage Collection (%)"], color='red', linestyle='--',
                              label='Reference MLE criterion\nwith single posterior\nULA sampling')
        else:
            axs[1, 0].axhline(y=ref["Garbage Collection (%)"], color='red', linestyle='--',
                              label='Reference ITS\nwith Mixture Prior')
    ...
    if ref is not None:
        if idx == 1:  # MALA steps plot
            axs[1, 1].axhline(y=ref["Allocations"], color='red', linestyle='--',
                              label='Reference MLE criterion\nwith single posterior\nULA sampling')
        else:
            axs[1, 1].axhline(y=ref["Allocations"], color='red', linestyle='--',
                              label='Reference ITS\nwith Mixture Prior')
