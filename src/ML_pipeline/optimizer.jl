...
function create_opt(conf::ConfParse)
    ...
    LR = parse(quant, retrieve(conf, "OPTIMIZER", "learning_rate"))
    m = parse(Int, retrieve(conf, "OPTIMIZER", "l-bfgs_memory"))
    c_1 = parse(quant, retrieve(conf, "LINE_SEARCH", "c_1"))
    c_2 = parse(quant, retrieve(conf, "LINE_SEARCH", "c_2"))
    ρ = parse(quant, retrieve(conf, "LINE_SEARCH", "rho"))
    ...
    optimiser_map = Dict(
        "bfgs" => BFGS(alphaguess=LineSearches.InitialHagerZhang{quant}(α0=LR), linesearch=linesearch),
        "l-bfgs" => LBFGS(alphaguess=LineSearches.InitialHagerZhang{quant}(α0=LR), m=m, linesearch=linesearch),
        "cg" => ConjugateGradient(alphaguess=LineSearches.InitialHagerZhang{quant}(α0=LR), linesearch=linesearch),
        "gd" => GradientDescent(alphaguess=LineSearches.InitialHagerZhang{quant}(α0=LR), linesearch=linesearch),
        "newton" => Newton(alphaguess=LineSearches.InitialHagerZhang{quant}(α0=LR), linesearch=linesearch),
